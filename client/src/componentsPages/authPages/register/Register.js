...
export const Register = () => {
  ...
  const changeHandler = event => {
    setForm({
      ...form,
      [event.target.name]: event.target.name === 'subscription'
  ...
  const registerHandler = async () => {
    ...
    try {
      const {
        ...
        subscription: testSubscription } = form
          ...
          <Grid container spacing={2}>
            ...
            <Grid item xs={12}>
              <FormControlLabel
                ...
                label="Я хочу получать на почту акционные предложение и другую полезную информацию для меня."
              />
            </Grid>
          </Grid>
          <Button
